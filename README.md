...
Downloads a URL, unzips it if it is a zip, then POSTs the absolute path of the unzipped folder to [bisque](https://github.com/gilmoregrills/bisque) or something else that accept [slskd]() style webhooks with the following body:

```json
{"localDirectoryName": "/path/to/folder"}
```
...
| NOTIFY_URL | URL to POST {"localDirectoryName": <absolute folder path>} to after a download |
